...
| 8 | Saveable setting variations + user defaults | 📋 | `card_photo_prefs` — **P2** |
...
| 11 | Photo quality options, which change space used | 📋 | Economy/Standard/High/Archive — **P2** |
...
### P2 — Settings, presets and defaults
*Idea 8 and 11. One table, one screen.*

- `card_photo_prefs`: capture mode, quality preset, auto-snap on/off, burst
  count, crop mode, margin %, keep-originals, default template
- Quality presets and what each costs in storage, **shown at the point of
  choice** rather than discovered on a bill
- Named bundles ("bulk intake" vs "consignment quality") so switching is one tap

Size: 6–10h. Depends on: nothing. Unblocks: P3, P4.

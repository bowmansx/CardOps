...
const sale = (o: Partial<CardApiSale>): CardApiSale => ({ price: 100, sold_at: "2026-05-01", title: "t", ...o });
...
describe("summarizeSales", () => {
  ...
  it("span_days from oldest→newest", () => {
    const s = summarizeSales([sale({ sold_at: "2026-01-01", price: 10 }), sale({ sold_at: "2026-01-31", price: 20 })]);
    expect(s.span_days).toBe(30);
  });
  it("sample is most-recent-first, capped at 8", () => {
    const many = Array.from({ length: 12 }, (_, i) => sale({ sold_at: `2026-05-${String(i + 1).padStart(2, "0")}`, price: 10 + i }));
